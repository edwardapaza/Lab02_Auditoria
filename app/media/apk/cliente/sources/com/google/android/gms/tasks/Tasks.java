...
public final class Tasks {
    ...
    public static <TResult> Task<List<TResult>> whenAllSuccess(Executor executor, Collection<? extends Task> collection) {
        return (collection == null || collection.isEmpty()) ? forResult(Collections.emptyList()) : (Task<List<TResult>>) whenAll(collection).continueWith(executor, new zzaa(collection));
    }
    ...
    public static <T> Task<T> withTimeout(Task<T> task, long j5, TimeUnit timeUnit) {
        q.l(task, "Task must not be null");
        q.b(j5 > 0, "Timeout must be positive");
        q.l(timeUnit, "TimeUnit must not be null");
