package com.google.android.gms.internal.p001firebaseauthapi;

import java.util.List;
import java.util.Map;
/* JADX INFO: Access modifiers changed from: package-private */
/* renamed from: com.google.android.gms.internal.firebase-auth-api.zzald  reason: invalid package */
/* loaded from: classes.dex */
public interface zzald {
    double zza();

    <T> void zza(T t5, zzalc<T> zzalcVar, zzaip zzaipVar);

    void zza(List<Boolean> list);

    @Deprecated
    <T> void zza(List<T> list, zzalc<T> zzalcVar, zzaip zzaipVar);

    <K, V> void zza(Map<K, V> map, zzakf<K, V> zzakfVar, zzaip zzaipVar);

    float zzb();

    <T> void zzb(T t5, zzalc<T> zzalcVar, zzaip zzaipVar);

    void zzb(List<zzahm> list);

    <T> void zzb(List<T> list, zzalc<T> zzalcVar, zzaip zzaipVar);

    int zzc();

    void zzc(List<Double> list);

    int zzd();

    void zzd(List<Integer> list);

    int zze();

    void zze(List<Integer> list);

    int zzf();

    void zzf(List<Long> list);

    int zzg();

    void zzg(List<Float> list);

    int zzh();

    void zzh(List<Integer> list);

    int zzi();

    void zzi(List<Long> list);

    int zzj();

    void zzj(List<Integer> list);

    long zzk();

    void zzk(List<Long> list);

    long zzl();

    void zzl(List<Integer> list);

    long zzm();

    void zzm(List<Long> list);

    long zzn();

    void zzn(List<String> list);

    long zzo();

    void zzo(List<String> list);

    zzahm zzp();

    void zzp(List<Integer> list);

    String zzq();

    void zzq(List<Long> list);

    String zzr();

    boolean zzs();

    boolean zzt();
}
